...
probe_parse_campos :: proc(t: ^testing.T) {
	d, _, c, w, h, _ := probe_parse("codec_name=h264\nwidth=1920\nheight=1080\nduration=123.5\n")
...
probe_parse_crlf_e_espacos :: proc(t: ^testing.T) {
	...
	d, _, c, _, _, _ = probe_parse("  codec_name=vp9  \n\n  duration=42.0  \n")
	testing.expect(t, c == "vp9" && t_feq(d, 42), "espaços e linhas em branco ignorados")
}

@(test)
probe_parse_duracao_na :: proc(t: ^testing.T) {
	d, _, c, _, _, _ := probe_parse("duration=N/A\ncodec_name=h264\n")
	testing.expect(t, d == 0 && c == "h264", "duração N/A não quebra o parse")
	d, _, c, _, _, _ = probe_parse("")
	testing.expect(t, d == 0 && c == "", "saída vazia = zero-values")
}

@(test)
probe_parse_duracao_stream_e_format :: proc(t: ^testing.T) {
	// stream=2001 + format=3965 (áudio mais longo, típico de live TikTok)
	d, vd, _, _, _, _ := probe_parse("codec_name=h264\nduration=2001.438\nwidth=640\nheight=1280\nduration=3965.485\n")
	testing.expect(t, t_feq(d, 3965.485), "dur (timeline) = maior = container/áudio")
	testing.expect(t, t_feq(vd, 2001.438), "v_dur = menor = stream de vídeo")
	d, vd, _, _, _, _ = probe_parse("duration=42.0\ncodec_name=h264\n")
	testing.expect(t, t_feq(d, 42) && t_feq(vd, 42), "uma duration= só: dur e v_dur iguais")
}
...
probe_parse_rotacao_troca_dims :: proc(t: ^testing.T) {
	...
	_, _, _, w, h, _ = probe_parse("width=1080\nheight=1920\nrotation=180\n")
...
probe_parse_fps :: proc(t: ^testing.T) {
	...
	_, _, _, _, _, f := probe_parse("codec_name=h264\navg_frame_rate=60/1\nr_frame_rate=60/1\n")
	testing.expect(t, t_feq(f, 60), "60/1 = 60fps")
	// 29.97: 30000/1001
	_, _, _, _, _, f = probe_parse("avg_frame_rate=30000/1001\n")
	testing.expect(t, f > 29.9 && f < 30.0, "30000/1001 ≈ 29.97fps")
	// avg "0/0" (VFR sem info) cai p/ r_frame_rate
	_, _, _, _, _, f = probe_parse("avg_frame_rate=0/0\nr_frame_rate=25/1\n")
